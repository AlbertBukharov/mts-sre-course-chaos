...
С помощью утилиты tc сэмулируем потери      
3. Ожидаемые результаты:
4. Реальные результаты:


Фиксируем потерю в 70%
```64 bytes from 10.0.10.4: icmp_seq=202 ttl=64 time=0.413 ms
^C
...
```
В дополнение к ожидаемому результату также фиксировалось отсутствие данных в Графане по проблемной ноде 10.0.10.4   
